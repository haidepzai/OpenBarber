...
const ShopReview = ({ review, onUpdated, onDeleted }) => {
  ...
  const avatarSrc = review.authorPhoto
    ? `data:image/jpeg;base64,${review.authorPhoto}`
    : AVATAR_URL;
  ...
  return (
    <Box sx={{ display: 'flex', gap: 4, borderBottom: 1, borderColor: 'divider', pb: 3 }}>
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'center' }}>
        ...
        />
        <Rating readOnly={isEditing ? false : true} precision={0.5} value={isEditing ? editRating : review.rating}
          onChange={(_, v) => setEditRating(v)}
          sx={{ color: 'primary.main' }} size="small" />
        ...
        {isEditing ? (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              ...
              <Button variant="outlined" size="small" onClick={() => { setIsEditing(false); setEditComment(review.comment); setEditRating(review.rating); }}>
                {t('CANCEL', 'Abbrechen')}
              </Button>
            </Box>
          </Box>
