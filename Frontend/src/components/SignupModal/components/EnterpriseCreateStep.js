...
const EnterpriseCreateStep = () => {
  ...
  function onBlur(e) {
    if (e.target.value === "") setErrors(err => ({...err, [e.target.name]: true }));
  }
  function update(e) {
    ...
    <Stack component="form" height="100%" gap={2} pt={16} onSubmit={onSubmit}>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "2fr 3fr 2fr", gap: 8 }}>
        ...
        <Stack gap={4}>
          ...
          <Stack gap={2} gridRow="2/4" gridColumn="3/4">
            <TextField label="Service Detail" variant="outlined" onBlur={onBlur} name="detail" error={errors.detail} value={newService.detail} onInput={update} />
            <Stack direction="row" gap={2}>
              <TextField label="Service Category" variant="outlined" sx={{ flexGrow: "1" }} onBlur={onBlur} name="category" error={errors.category} value={newService.category} onInput={update}  />
              <TextField label="Service Price" variant="outlined" sx={{ flexGrow: "1" }} onBlur={onBlur} name="price" error={errors.price} value={newService.price} onInput={update}  />
            ...
            <Stack gap={2} overflow="hidden auto" pl={1}>
              {data.shopServices.map((service, index) => (
                <Stack direction="row" key={index} gap={2} alignItems="center">
                  ...
                  <Typography fontSize={12} mr="auto" color="secondary.main">{service.price}</Typography>
                  <Typography fontSize={10} height={20} sx={{ backgroundColor: "primary.main", color: "primary.contrastText", paddingX: 1, borderRadius: 100, display: "flex", alignItems: "center" }}>{service.category}</Typography>
                  
                  <Button size="small" variant="outlined" onClick={() => setData((d) => ({ ...d, shopServices: d.shopServices.filter((_, i) => i !== index) }))}>Remove</Button>
                </Stack>
              ))}
            </Stack>
          </Stack>
      ...
      <Stack direction="row" justifyContent="space-between" marginTop="auto" width="100%" gap={2}>
        ...
        <Button type="submit" disabled={
          !(data.enterpriseName && data.firstShopName && data.shopEmail && data.shopPhone && data.shopHeaderUrl && data.shopDescription && data.shopWebsite && data.shopServices.length > 0)
        } variant="contained">
          Continue
        </Button>
      </Stack>
