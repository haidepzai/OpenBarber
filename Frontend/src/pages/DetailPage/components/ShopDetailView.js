...
const ShopDetailView = ({ shop }) => {
  ...
  return (
    <Box className="shopDetailView-container">
          ...
          <TabPanel value={tab} index={1} sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
            {
              shop.services.map((service, i) => (
                <Box key={i} sx={{ display: "flex", alignItems: "center", gap: 2, backgroundColor: "grey.200", borderRadius: "100vw", padding: 2, paddingBottom: 1, paddingTop: 1}}>
                  <Typography variant="span" color="primary.main" sx={{ fontWeight: 600 }}>{service.price}&euro;</Typography>
      ...
      <Typography variant="h4" mt={4}>Reviews</Typography>
      <Box sx={{ display: "flex", flexDirection: "column" }}>
        {shop.reviews.map(review => (
          <Box sx={{ display: "flex", gap: 4, paddingTop: 3, paddingBottom: 3, borderBottom: 1, borderColor: "divider" }}>
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2, alignItems: "center" }}>
              <Box boxShadow={2} sx={{ width: "5rem", height: "5rem", borderRadius: "50%", backgroundImage: `url(${AVATAR_URL})`, backgroundSize: "cover" }}></Box>
              <Rating readOnly precision={0.5} value={review.rating} sx={{ color: "primary.main" }} size="small" />
              <Typography variant="span" color="grey.400">{review.date}</Typography>
            </Box>
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <Typography variant="h5">
                <Typography variant="span" sx={{ fontWeight: 600 }}>{review.name}</Typography>
                &nbsp;wrote:
              </Typography>
              <Typography variant="body1">{review.comment}</Typography>
              <Typography variant="span" color="grey.400" mt="auto">{review.name} has added 0 photos to the gallery.</Typography>
            </Box>
          </Box>
        ))}
