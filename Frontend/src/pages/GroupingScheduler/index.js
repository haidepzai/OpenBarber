...
const getServiceInstances = (services) => [...new Map(services.map((service) => ({ id: service.id, text: service.title, color: 'blue' })).map((e) => [e.id, e])).values()]
const getEmployeeInstances = (employees) => [...new Map(employees.map((employee) => ({ id: employee.id, text: employee.name })).map((e) => [e.id, e])).values()]
    ...
    return (
        <AppointmentTooltip.Content {...restProps} appointmentResources={displayData(appointmentResources)} appointmentData={appointmentData}>
            <Grid container alignItems="center">
                ...
                <Grid item xs={10}>
                    <div>Customer: {appointmentData.customer.firstName} {appointmentData.customer.lastName}</div>
                </Grid>
            </Grid>
...
const GroupingSchedulerPage = () => {
    ...
    const filterAppointments = (appointments) => appointments.filter((appointment) =>
        (!currentEmployee || appointment.employee === currentEmployee)
    )
    ...
    const changeCurrentEmployee = (newCurrentEmployee) => {
        const newResources = resources.map((resource) => {
            if (resource.fieldName === "employee") {
                return {
                    ...resource,
                    instances: newCurrentEmployee ? [getEmployeeInstances(allEmployees).find((e) => e.id === newCurrentEmployee)] : getEmployeeInstances(allEmployees)
                }
            } else {
                return resource;
            }
                            ...
                            flexibleSpaceComponent={() => (
                                <Toolbar.FlexibleSpace style={{ margin: '0 auto 0 0', display: "flex", gap: "40px", alignItems: "center" }}>
                                    <EmployeeSelect
                                        ...
                                        handleChange={changeCurrentEmployee}
                                        options={getEmployeeInstances(allEmployees)}
                                    />
                                    <FormGroup>
